...
import constant
from func import get_trigger_batch,get_batch,get_trigger_feeddict,f_score,get_argument_feeddict,GAC_func,Cudnn_RNN,matmuls
import numpy as np
class DMCNN():
    ...
    def build_graph(self):
        if self.stage=='trigger':
            ...
            self.build_argument()
        elif self.stage=="WAHM":
            print('--Building WAHM Graph--')
            self.build_WAHM()
        else:
            raise ValueError("stage could only be trigger or DMCNN or WAHM")

    def build_trigger(self,scope='SCGAT_Trigger'):
        maxlen = self.maxlen
        ...
        with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
            with tf.variable_scope('Initialize'):
                ...
                pos_mat = tf.concat([
                    ...
                    tf.get_variable("pos_embedding", [len(constant.POS_TO_ID) - 1, constant.pos_dim], dtype=tf.float32,
                                    initializer=tf.contrib.layers.xavier_initializer())], axis=0)
                ner_mat = tf.concat([
                    tf.zeros((1, constant.ner_dim), dtype=tf.float32),
                    tf.get_variable("ner_embedding", [len(constant.NER_TO_ID) - 1, constant.ner_dim], dtype=tf.float32,
                                    initializer=tf.contrib.layers.xavier_initializer())], axis=0)
            ...
            with tf.variable_scope("Lstm_layer"):
                ...
                ps, _ = rnn(concat_emb, seq_len=sents_len, concat_layers=False, keep_prob=keepprob,
                            is_train=self.is_train)

            with tf.variable_scope("GAC"):
                hs = []
                for layer in range(1, constant.K + 1):
                    h_layer = GAC_func(ps, matmuls(subg_a, layer)+eyes, maxlen, 'a', layer) + GAC_func(ps, matmuls(subg_b,layer)+eyes,maxlen, 'b',layer) + GAC_func(ps,eyes,maxlen,'c',layer)
                    hs.append(h_layer)
            ...
            with tf.variable_scope('classifier'):
                ...
                self.logits = logits = tf.layers.dense(gather_final_h, num_class,
                ...
                self.loss = loss = tf.reduce_sum(
                    bias_weight * tf.nn.softmax_cross_entropy_with_logits_v2(labels=labels, logits=logits),
    ...
    def train_trigger(self):
        ...
        with tf.Session() as sess:
            ...
            #     loss_list = []
            #     for batch in get_trigger_batch(train, constant.t_batch_size, True):
            #         loss, _ = sess.run([self.loss, self.train_op],
            #                            feed_dict=get_trigger_feeddict(self, batch, self.stage, maxlen))
            #         loss_list.append(loss)
            #     print('epoch:{}'.format(str(epoch)))
            #     print('loss:', np.mean(loss_list))
            #
            #     pred_labels = []
            #     for batch in get_trigger_batch(dev, constant.t_batch_size, False):
            ...
            #
            #     pred_labels = []
            #     for batch in get_trigger_batch(test, constant.t_batch_size, False):
            #         pred_label = sess.run(self.pred_label,feed_dict=get_trigger_feeddict(self, batch, self.stage, maxlen,is_train=False))
            #         pred_labels.extend(list(pred_label))
            #     golds = list(test[0][4])
    ...
    def predict_trigger(self,sess):
        ...
        for batch in get_trigger_batch(dev,constant.t_batch_size,False):
            pred_label = list(sess.run(self.pred_label,feed_dict=get_argument_feeddict(self,batch,maxlen,False,'trigger')))
    ...
    def process_data_for_argument(self,dev_pred_event_types,test_pred_event_types):
        ...
        train, subtrain = list(self.a_train)
        dev, subdev = list(self.a_dev)
        test, subtest = list(self.a_test)
        print('dev event type: ', dev_pred_event_types)
        print('test event type: ', test_pred_event_types)
    ...
    def build_WAHM(self,scope="WAHM"):
        maxlen = self.maxlen
        num_class = len(constant.ROLE_TO_ID)
        with tf.variable_scope(scope,reuse=tf.AUTO_REUSE):
            with tf.variable_scope('Initialize'):
                ...
                module_weight = tf.constant(constant.module_weight,tf.float32)
            with tf.variable_scope('placeholder'):
                self.sents = sents = tf.placeholder(tf.int32,[None,maxlen],'sents')
                self.trigger_posis = trigger_posis = tf.placeholder(tf.int32,[None,maxlen],'trigger_posis')
                self.argument_posis = argument_posis = tf.placeholder(tf.int32,[None,maxlen],'argument_posis')
            ...
            with tf.variable_scope("attention"):
                ...
                hidden_state = tf.layers.dense(tf.concat([conv_res_extend,u_c_feat],axis=3),constant.a_W_a_dim,use_bias=False,kernel_initializer=tf.contrib.layers.xavier_initializer(),activation=tf.nn.tanh)#tanh
                ...
                module_weight_mask = tf.tile(tf.expand_dims(tf.expand_dims(tf.expand_dims(module_weight,axis = 0),axis = 2), axis = 3),[batch_size,1,constant.module_num,maxlen])
                score_mask = tf.tile(tf.expand_dims(module_score,axis=1),[1,len(constant.ROLE_TO_ID),1,1])*module_mask*module_weight_mask
                # module_of_role = tf.expand_dims(tf.expand_dims(tf.reduce_sum(module_design,axis=1),axis=0),axis=2)
                module_of_weight = tf.expand_dims(tf.expand_dims(module_weight, axis=0), axis=2)
                # print("module_of_role: " , type(module_of_role) , " / " , module_of_role)
                # role_score = tf.reduce_sum(score_mask,axis=2)/module_of_role
                role_score = tf.reduce_sum(score_mask, axis=2) / module_of_weight
            ...
            with tf.variable_scope('classifier'):
                ...
                wahm_feature = tf.concat([tf.tile(tf.expand_dims(dmcnn_feature,axis=1),[1,len(constant.ROLE_TO_ID),1]),role_oriented_emb],axis=2)
                feature = tf.layers.dropout(wahm_feature,1-constant.a_keepprob,training=is_train)
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(constant.a_epoch):
                ...
                for batch in get_batch(train,constant.a_batch_size,shuffle=True):
                    loss,_ = sess.run([self.loss,self.train_op],feed_dict=get_argument_feeddict(self,batch,maxlen,True,"argument"))
                    loss_list.append(loss)
                print('epoch:{}'.format(str(epoch)))
                print('loss:',np.mean(loss_list))
                ...
                for batch in get_batch(dev,constant.a_batch_size,False):
                    pred_event_types,feed_dict = get_argument_feeddict(self,batch,maxlen,False,"argument")
